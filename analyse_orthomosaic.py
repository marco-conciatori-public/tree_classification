...
species_distribution = np.zeros((total_height, total_width, meta_data['num_classes'] + 2), dtype=np.int8)
...
for x in range(0, x_max, stride):
    for y in range(0, y_max, stride):
        ...
        print('species_distribution[y: y + patch_size, x: x + patch_size, : ] shape: ')
        print(f'{species_distribution[y: y + patch_size, x: x + patch_size, : ].shape}')
        print(f'{species_distribution[y: y + 5, x: x + 5, : ]}')
